...
func (p *createAPISubcommand) InjectResource(res *resource.Resource) error {
	...
	// Same validation as go/v4: do not scaffold over existing API unless --force
	if r, err := p.config.GetResource(p.resource.GVK); err == nil && r.HasAPI() && !p.force {
		return errors.New("API resource already exists")
	}

	// Same validation as go/v4: reject new group when project is single-group
...
func (p *createAPISubcommand) PreScaffold(machinery.Filesystem) error {
	...
	for _, pluginKey := range p.config.GetPluginChain() {
		if strings.Contains(pluginKey, "deploy-image") {
			return fmt.Errorf("ssa plugin cannot be used with deploy-image plugin. " +
...
func (p *createAPISubcommand) Scaffold(fs machinery.Filesystem) error {
	isFirstAPI := p.isFirstServerSideApplyAPI()
	scaffolder := scaffolds.NewAPIScaffolder(p.config, *p.resource, isFirstAPI)
	...
	// Save resource info to PROJECT file (must happen in Scaffold, not PostScaffold)
...
// isFirstServerSideApplyAPI returns true if the PROJECT file does not yet contain the
// ssa plugin or has no resources from it (first API with this plugin).
// Only then we add the applyconfiguration target to the Makefile.
func (p *createAPISubcommand) isFirstServerSideApplyAPI() bool {
	key := plugin.KeyFor(Plugin{})
	cfg := PluginConfig{}
	if err := p.config.DecodePluginConfig(key, &cfg); err != nil {
		return true
	}
	return len(cfg.Resources) == 0
}

// PostScaffold can be removed since we save in Scaffold() now
