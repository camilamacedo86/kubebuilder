...
# Update the Helm chart under the dist/ directory and overwrite all files
  %[1]s edit --plugins=helm/v1-alpha --force=true

**IMPORTANT**: If the "--force=true" flag is not used, the following files will not be updated to preserve your customizations:
dist/chart/
├── values.yaml
└── templates/
    └── manager/
