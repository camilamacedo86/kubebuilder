...
func MakeContainerArgsConditional(yamlContent string) string {
	...
	// Make metrics-cert-path arg conditional on certManager.enabled AND metrics.enabled
	if strings.Contains(yamlContent, "--metrics-cert-path") {
		...
		yamlContent = metricsArgPattern.ReplaceAllStringFunc(yamlContent, func(match string) string {
			...
			argLine := strings.TrimSpace(match)
			return fmt.Sprintf("%s{{- if and .Values.certManager.enabled .Values.metrics.enabled }}\n%s%s\n%s{{- end }}",
				indent, indent, argLine, indent)
		})
	}
...
// MakeMetricsVolumesConditional makes metrics volumes conditional on certManager.enabled AND metrics.enabled.
func MakeMetricsVolumesConditional(yamlContent string) string {
	// Make metrics volumes conditional on certManager.enabled AND metrics.enabled
	if strings.Contains(yamlContent, "metrics-certs") && strings.Contains(yamlContent, "secretName: metrics-server-cert") {
		...
		yamlContent = volumePattern.ReplaceAllStringFunc(yamlContent, func(match string) string {
			...
			if len(lines) > 0 {
				...
				fmt.Fprintf(&result, "%s{{- if and .Values.certManager.enabled .Values.metrics.enabled }}\n", childIndent)
...
// MakeMetricsVolumeMountsConditional makes metrics volumeMounts conditional on certManager.enabled AND metrics.enabled.
func MakeMetricsVolumeMountsConditional(yamlContent string) string {
	// Make metrics volumeMounts conditional on certManager.enabled AND metrics.enabled
