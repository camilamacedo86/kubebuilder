...
function scaffold_test_project {
  ...
  if [ $project == "project-v4" ] ; then
    ...
    $kb create api --group "cert-manager.io" --version v1 --kind Certificate --controller=true --resource=false --make=false --external-api-path=github.com/cert-manager/cert-manager/pkg/apis/certmanager/v1 --external-api-domain=""
