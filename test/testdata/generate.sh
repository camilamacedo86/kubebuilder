...
function scaffold_test_project {
  ...
  if [[ $project =~ multigroup ]]; then
    ...
    $kb create api --group sea-creatures --version v1 --kind Prawn --plugins="ssa/v1-alpha" --controller=true --resource=true --make=false
  fi
  ...
  if [[ $project =~ multigroup ]] || [[ $project =~ with-plugins ]] ; then
    ...
    $kb create api --group example.com --version v1 --kind Application --plugins="ssa/v1-alpha" --controller=true --resource=true --make=false
