...
var _ = Describe("kubebuilder", func() {
	Context("plugin helm/v2-alpha", func() {
		...
		It("should work when chart name is customized via fullnameOverride", func() {
			...
			By("customizing resource names via fullnameOverride following Helm best practices")
...
// The resource names use chart.serviceName which handles 63-char truncation.
// In E2E tests, truncation never occurs because test suffix is only 4 chars (e.g., "abcd"),
// resulting in short base names like "e2e-abcd" or "custom-operator".
func runHelm(kbc *utils.TestContext, hasWebhook, hasMetrics, hasNetworkPolicies bool, expectFullnameOverride string) {
	// Determine expected resource prefix.
	// Note: In E2E, chart.serviceName won't truncate because base names are short (< 20 chars)
	expectedPrefix := "e2e-" + kbc.TestSuffix
	if expectFullnameOverride != "" {
		expectedPrefix = expectFullnameOverride
	}

	// Update ServiceAccount name to match the expected prefix
	// This is critical because the Helm chart templates resource names using {{ include "chart.fullname" . }}
	kbc.Kubectl.ServiceAccount = expectedPrefix + "-controller-manager"
	...
	if hasWebhook {
		...
		verifyCAInjection := func(g Gomega) {
			...
			mwhOutput, err = kbc.Kubectl.Get(
				...
				fmt.Sprintf("%s-mutating-webhook-configuration", expectedPrefix),
			...
			vwhOutput, err = kbc.Kubectl.Get(
				...
				fmt.Sprintf("%s-validating-webhook-configuration", expectedPrefix),
				"-o", "go-template={{ range .webhooks }}{{ .clientConfig.caBundle }}{{ end }}")
			g.Expect(err).NotTo(HaveOccurred())
			// check that ca should be long enough, because there may be a place holder "\n"
...
func getMetricsOutput(kbc *utils.TestContext, expectedPrefix string, controllerPodName string) string {
	...
	checkServiceEndpoint := func(g Gomega) {
		...
		output, err = kbc.Kubectl.Get(
			true,
			"endpoints", fmt.Sprintf("%s-controller-manager-metrics-service", expectedPrefix),
	...
	cmdOpts := cmdOptsToCreateCurlPod(kbc, token, expectedPrefix)
...
func cmdOptsToCreateCurlPod(kbc *utils.TestContext, token, expectedPrefix string) []string {
	//nolint:lll
	cmdOpts := []string{
		"run", "curl",
		"--restart=Never",
    ...
    }`, token, expectedPrefix, kbc.Kubectl.Namespace, kbc.Kubectl.ServiceAccount),
	}
	return cmdOpts
}
