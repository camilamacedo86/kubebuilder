...
- If you need to update the Helm chart generated under `dist/chart/` with the latest project changes:
  ...
  you must run the `edit` command with the `--force=true` flag to regenerate the Helm chart values based
...
<aside class="note">
  ...
  The files `chart/Chart.yaml`, `chart/templates/_helpers.tpl`, and `chart/.helmignore` are never updated after their initial creation.
